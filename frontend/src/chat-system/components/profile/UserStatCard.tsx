...
export function UserStatCard({ userGameStat, isOwnProfile }: UserStatCardProps) {
    ...
    const winRate = totalGames > 0 ? Math.round(((userGameStat?.wins ?? 0) / totalGames) * 100) : 0;
    return (
        <div className="relative group/stats overflow-hidden">
            ...
            <div className="bg-slate-900/60 border border-white/10 backdrop-blur-2xl rounded-4xl p-6 md:p-10 shadow-2xl">
                ...
                {/* Progress Visualizer */}
                <div className="bg-slate-800/30 border border-white/5 p-6 md:p-8 rounded-4xl backdrop-blur-md relative overflow-hidden">
                    ...
                    <div className="relative h-6 w-full bg-slate-950/50 rounded-full border border-white/5 p-1 overflow-hidden shadow-inner">
                        <div
                            className="h-full bg-linear-to-r from-emerald-600 via-emerald-400 to-cyan-400 rounded-full shadow-[0_0_20px_rgba(52,211,153,0.4)] transition-all duration-[2000ms] ease-out"
                            style={{ width: `${winRate}%` }}
                        >
