...
function ChatWindow({ onFirstMessage, initialMessages = [], sessionId }: ChatWindowProps) {
  ...
  const handleSend = async () => {
    ...
    const checkIfImageRequest = async (message: string): Promise<boolean> => {
      try {
        const res = await fetch('/chatbot/is-image-request', {
          method: 'POST',
          headers: jsonHeaders,
          credentials: 'include',
        ...
        const data = await res.json()
        return data.is_image === true
      } catch {
        return false
      }
    }
    
    const isImageRequest = await checkIfImageRequest(messageText)
    ...
    if (isImageRequest) {
      try {
        ...
        if (!response.ok || !data.image_url) {
          setMessages(prev => [...prev, { role: 'ai', text: data.error ?? 'Error generating image.' }])
        } else {
          const imageHtml = '<img src="' + data.image_url + '" alt="Generated image" class="max-w-full rounded-lg border border-blue-800" />'
  ...
  const generateTitle = async (message: string) => {
    try {
      ...
      if (onFirstMessage) onFirstMessage(data.title ?? 'New Chat')
    } catch {
  ...
  return (
    <div className="flex flex-1 h-full flex-col bg-slate-900/80 border border-blue-800 rounded-2xl overflow-hidden shadow-2xl">
      <div
      ...
      >
        {messages.length === 0 && !loading && (
          <div className="text-center pt-16 pb-5">
            <div className="inline-block rounded-xl bg-slate-800 border border-blue-700 text-amber-400 px-6 py-4 text-[18px]">
            ...
            {msg.role === 'ai' && (
              <div className="flex flex-col gap-2 max-w-160">
                <div className="flex items-center gap-2">
                  <div className="w-7 h-7 rounded-lg bg-amber-500/20 border border-amber-500/30 flex items-center justify-center text-[13px] text-amber-300">♟</div>
      ...
      <div className="px-10 pb-4 pt-3 border-t border-blue-800 bg-slate-900/70">
        {uploadedFile && (
          <div className="flex items-center gap-2 px-3 py-1.5 mb-2 bg-slate-800 border border-blue-700 rounded-lg w-fit text-xs text-slate-200">
            ...
            <button className="text-red-400 hover:text-red-300 text-sm" onClick={removeFile}>✕</button>
          </div>
        )}
        <div className="flex items-center bg-slate-900 border border-blue-700 rounded-xl px-3 py-2 gap-3 transition-colors focus-within:border-amber-400/60">
          ...
          <button className="text-[16px] text-slate-300 hover:text-amber-300 p-1.5 rounded-md" onClick={handleAttachClick} title="Attach file">📎</button>
          <textarea
